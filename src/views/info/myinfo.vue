            ...
            <div :class="themeStore.currentTheme === 'dark' ? 'modal-content-dark' : 'modal-content'">
              ...
              <form @submit.prevent="updateUserInfo">
                ...
                <div class="form-group">
                  ...
                  <input type="text" id="college" v-model="editData.college">
                </div>
      ...
      <div class="info-grid">
        <div v-for="(item, index) in filteredInfo" :key="item.label" class="info-card"
          ...
          <div class="info-content">
            ...
            <div class="info-value">{{ item.value || '无' }}</div>
          </div>
        </div>
      </div>
    </div>
...
const processedAvatar = computed(() => {
  if (avatarError.value) return '/default-avatar.jpg'
...
const updateUserInfo = async () => {
  try {
    ...
    if (!editData.value.username || !editData.value.email ||
      !editData.value.phone || !editData.value.cn_1 ||
      !editData.value.college) {
    ...
    // 调用API更新用户信息
    const response = await infoAPI.updateStudentInfo(userId, updateData);
...
const goToSettings = () => {
  // 这里可以添加跳转到设置页面的逻辑
  // 例如使用 vue-router 进行页面跳转
  // 假设我们有一个名为 Settings 的路由
  // router.push('/settings');

  // 暂时代替实现 - 显示一个消息提示
  ElMessage.info('设置功能暂未实现，敬请期待');
}
