<template>
  <div class="user-info-container" :class="{ 'dark': themeStore.isDark }">
    ...
    <div class="profile-header">
      <h1>我的比赛</h1>
      <button v-if="role == 'student'" class="simple-icon-only" @click="showCreateTeamModal" data-text="创建新比赛">
        <span class="glow"></span>
        <span class="content">
          <img :src="newteamIconSrc" alt="创建新比赛" class="action-icon tiny" />
        </span>
      </button>
    </div>
    ...
    <div class="info-panel">
      <div class="team-list">
        <template v-if="teams.length > 0">
          <router-link v-for="(team, index) in teams" :key="team.id" :to="`/competition/detail/${team.id}`"
            class="team-card" @click.native.prevent="handleTeamCardClick(team)"
            @contextmenu.prevent="handleRightClick(team)">
            <div class="border-effect"></div>
            <div class="team-content">
              <h3 class="team-name">{{ team.title }}</h3>
              <div class="team-info">描述: {{ team.description }}</div>
              <div class="team-info">奖项: {{ team.score }}</div>
              <div class="team-info">备注: {{ team.note }}</div>
        ...
        <div v-else class="empty-state">
          ...
          <h3 class="empty-title">暂时未有比赛</h3>
          <p class="empty-tip">点击右上角按钮创建你的第一个比赛</p>
        </div>
      </div>
    </div>
              ...
              <div class="member-list">
                <div v-for="student in statusData.students" :key="student.sn" class="member-item">
                  ...
                  <div v-if="isCurrentUser(student) && student.status === 'pending'" class="action-buttons">
                    <button class="cyber-button success" @click="updateStudentStatus('confirmed', student)">
                      <span class="content">确认</span>
                    </button>
                    <!-- <button class="cyber-button danger" @click="updateStudentStatus('rejected', student)">
            ...
            <div class="members-section">
              ...
              <div class="member-list">
                <div v-for="teacher in statusData.teachers" :key="teacher.sn" class="member-item">
                  ...
                  <!-- 状态标签仅对非当前用户显示 -->
                  <span v-if="!isCurrentUser(teacher) || ['confirmed', 'rejected'].includes(teacher.status)"
                    :class="['status-tag', teacher.status]">
                    {{ getStatusText(teacher.status) }}
                  </span>

                  <!-- 当前用户操作按钮 -->
                  <div v-if="isCurrentUser(teacher) && teacher.status === 'pending'" class="action-buttons">
                    <button class="cyber-button success" @click="updateTeacherStatus('confirmed', teacher)">
                      <span class="content">确认</span>
                    </button>
                    <!-- <button 
        ...
        <div class="modal-overlay" @click.self="hideCreateTeamModal">
          <div :class="themeStore.currentTheme === 'dark' ? 'modal-content-dark' : 'modal-content'">
            ...
            <div class="form-group">
              <label for="newCompetitionTitle">比赛名称:</label>
              <input type="text" id="newCompetitionTitle" v-model="newCompetition.title" required />
            </div>

            <!-- 输入比赛日期 -->
            <div class="form-group">
              <label for="newCompetitionDate">比赛日期:</label>
              <input type="date" id="newCompetitionDate" v-model="newCompetition.date" required />
            </div>

            <!-- 输入比赛描述 -->
            <div class="form-group">
              <label for="newCompetitionDescription">比赛描述:</label>
              <textarea id="newCompetitionDescription" v-model="newCompetition.description" required></textarea>
            </div>
            ...
            <!-- 选择比赛文件 -->
            <div class="form-group">
              <label for="newCompetitionCertificate">比赛文件:</label>
              <input type="file" id="newCompetitionCertificate" @change="handleFileChange" />
            </div>
            ...
            <div class="form-group">
              ...
              <div class="selected-members">
                <div v-if="selectedTeam" class="member-tag">
                  {{ selectedTeam.name }}
                  <span class="remove-btn" @click="removeTeam">×</span>
                </div>
              </div>
              <button class="add-member-btn" @click="showTeamSelector = true">
                + 选择团队
              </button>
            </div>
            ...
            <div class="form-group">
              <label>带队老师:</label>
              <div class="selected-members">
                <div v-for="teacher in selectedTeachers" :key="teacher.sn" class="member-tag">
                  {{ teacher.username }}
            ...
            <div class="modal-actions">
              <button type="submit" @click="createCompetition">创建比赛</button>
    ...
    <!-- 团队选择弹窗 -->
    <Teleport to="body">
      <div v-if="showTeamSelector" class="selector-modal">
        <div class="modal-overlay" @click.self="closeTeamSelector">
          <div :class="themeStore.currentTheme === 'dark' ? 'modal-content-dark' : 'modal-content'">
            <h3>选择团队</h3>
            <div class="search-box">
              <input type="text" v-model="teamSearchQuery" placeholder="按团队名称搜索..." @input="fetchMyTeams" />
            </div>
            <div class="members-grid">
              <div v-for="team in myTeams" :key="team.id" class="member-card selectable"
                :class="{ selected: isTeamSelected(team) }" @click="selectTeam(team)">
                <div class="member-info">
                  <strong>团队名: {{ team.name }}</strong>
                  <!-- <p>队长: {{ team.cap_name }}</p> -->
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-if="showTeacherSelector" class="selector-modal">
        <div class="modal-overlay" @click.self="closeTeacherSelector">
          <div :class="themeStore.currentTheme === 'dark' ? 'modal-content-dark' : 'modal-content'">
            <h3>选择教师</h3>
            <div class="search-box">
              <input type="text" v-model="teacherSearchQuery" placeholder="按姓名或工号搜索..." @input="fetchAllTeachers" />
            </div>
            <div class="members-grid">
              <!-- 修改为直接遍历 allTeachers -->
              <div v-for="teacher in allTeachers" :key="teacher.sn" class="member-card selectable"
                :class="{ selected: isTeacherSelected(teacher) }" @click="toggleTeacherSelection(teacher)">
                <img :src="getAvatarUrl(teacher.avatar)" alt="头像" class="member-avatar" />
                <div class="member-info">
...
const handleTeamCardClick = (team) => {
  ...
  console.log('点击了团队卡片', team)
  if (team.status !== 'pending') {
    router.push(`/competition/detail/${team.id}`)
  }
  handlePendingStatusClick(team)
}
...
const createCompetition = async () => {
  ...
  formData.append('title', newCompetition.value.title)
  formData.append('date', newCompetition.value.date)
  formData.append('description', newCompetition.value.description)
  formData.append('score', newCompetition.value.score)
  if (newCompetition.value.certificate_image) {
    formData.append('certificate_image', newCompetition.value.certificate_image)
  }
  formData.append('team_sn', selectedTeam.value.sn)
  selectedTeachers.value.forEach(teacher => {
    formData.append('teacher_ids', teacher.sn)
  })

  try {
    const response = await infoAPI.createCompetition(formData)
    if (response.status === 201) {
      ElMessage.success('比赛创建成功')
      hideCreateTeamModal()
      fetchcompetitionData()
    } else {
      ElMessage.error(response.data.message || '比赛创建失败')
    }
  } catch (error) {
    console.error('创建比赛失败:', error)
    ElMessage.error('创建比赛失败，请稍后再试')
  }
}

// 处理待确认状态点击事件
const handlePendingStatusClick = async (team) => {
  console.log('点击了待确认状态')
...
// 显示确认按钮
const showConfirmButtons = (team) => {
  team.showConfirm = !team.showConfirm
}

// 更新团队状态
const updateTeamStatus = async (status, team) => {
  try {
    const response = await infoAPI.updateTeamStatus({
      competition_sn: team.sn,
      status: status
    })

    if (response.data.code === 200) {
      ElMessage.success(status === 'confirmed' ? '确认成功' : '拒绝成功')
      team.status = status
...
const handleFileChange = (event) => {
  newCompetition.value.certificate_image = event.target.files[0]
}

// 过滤团队
const filteredTeams = computed(() => {
  if (!teamSearchQuery.value) return myTeams.value
  return myTeams.value.filter(team =>
    team.title.toLowerCase().includes(teamSearchQuery.value.toLowerCase())
  )
})
// 选择团队
...
const isTeamSelected = (team) => {
  return selectedTeam.value && selectedTeam.value.id === team.id
}
// 关闭团队选择弹窗
...
// 获取团队确认状态
const fetchTeamConfirmStatus = async (competition_sn) => {
  try {
    const response = await infoAPI.CompetitionConfirmStatus(competition_sn)
...
const fetchAllTeachers = async () => {
  try {
    ...
    console.log('教师列表:', allTeachers.value)
  } catch (error) {
...
const removeTeacher = (teacher) => {
  const index = selectedTeachers.value.findIndex(t => t.sn === teacher.sn)
  if (index !== -1) {
    selectedTeachers.value.splice(index, 1)
  }
}
...
.team-card {
  /* 增加内边距来提高行高 */
  padding: 1.8rem;
  /* 原 padding 是 1.2rem，现在增加到 1.8rem */
  /* 或者直接设置最小高度 */
  min-height: 150px;
  /* 根据需要调整高度 */
...
.selector-actions .cyber-button.success,
