<template>
    <div class="user-info-container" :class="{ 'dark': themeStore.isDark }">
        ...
        <div class="profile-header">
            <h1>我的专利</h1>
            <button v-if="role == 'student'" class="simple-icon-only" @click="showCreatePatentModal" data-text="创建新专利">
                <span class="glow"></span>
                <span class="content">
                    <img :src="newteamIconSrc" alt="创建新专利" class="action-icon tiny" />
                </span>
            </button>
        </div>
        <div class="search-bar">
            <input type="text" v-model="searchQuery" @input="debouncedSearch" placeholder="搜索专利名称"
                class="search-input" />
        </div>
        ...
        <div class="info-panel">
            <div class="team-list">
                <template v-if="patents.length > 0">
                    <router-link v-for="patent in patents" :key="patent.id" :to="`/patent/detail/${patent.id}`"
                        class="team-card">
                        <div class="border-effect"></div>
                        <div class="team-content">
                            <h3 class="team-name">{{ patent.name }}</h3>
                            <div class="team-info">专利号: {{ patent.number }}</div>
                            <div class="team-info">类型: {{ patent.patent_type }}</div>
                            <div class="team-info">申请日期: {{ patent.date }}</div>
                            <div class="team-info">申请人: {{ patent.applicant_name }}</div>
                        </div>
                    </router-link>
                </template>
                <div v-else class="empty-state">
                    ...
                    <h3 class="empty-title">暂无专利数据</h3>
                    <p class="empty-tip">点击右上角按钮创建你的第一个专利</p>
                </div>
            </div>
        </div>
                ...
                <div class="modal-overlay" @click.self="closeModal">
                    <div class="modal-content-dark">
                        ...
                        <div class="modal-scroll-container">
                            <form @submit.prevent="submitPatent">
                                ...
                                <div class="form-group">
                                    <label for="newPatentName">专利名称:</label>
                                    <input type="text" id="newPatentName" v-model="newPatent.name" required />
                                </div>

                                <!-- 专利号 -->
                                <div class="form-group">
                                    <label for="newPatentNumber">专利号:</label>
                                    <input type="text" id="newPatentNumber" v-model="newPatent.number" required />
                                </div>

                                <!-- 申请日期 -->
                                <div class="form-group">
                                    <label for="newPatentDate">申请日期:</label>
                                    <input type="date" id="newPatentDate" v-model="newPatent.date" required />
                                </div>

                                <!-- 专利类型 -->
                                <div class="form-group">
                                    <label for="newPatentType">专利类型:</label>
                                    <select id="newPatentType" v-model="newPatent.patent_type" required>
                                ...
                                <!-- 专利描述 -->
                                <div class="form-group">
                                    <label for="newPatentDescription">专利描述:</label>
                                    <textarea id="newPatentDescription" v-model="newPatent.description"
                                        required></textarea>
                                </div>

                                <!-- 专利文件 -->
                                <div class="form-group">
                                    <label for="newPatentFile">专利文件(PDF):</label>
                                    <input type="file" id="newPatentFile" @change="handleFileUpload" accept=".pdf"
                                        required />
                                </div>
                                ...
                                <div class="form-group">
                                    <label>发明人:</label>
                                    <div class="inventors-scroll-container">
                                        <div v-for="(inventor, index) in newPatent.inventors" :key="index"
                                            class="inventor-item">
                                            {{ inventor.name }}
                                            <span class="remove-btn" @click="removeInventor(index)">x</span>
                                        </div>
                                    </div>
                                    <button type="button" @click="addInventor" class="add-member-btn">
                                        + 添加发明人
                                    </button>
                                </div>
                            </form>
                        ...
                        <div class="modal-actions">
                            <button type="submit" @click="submitPatent">提交</button>
        ...
        <Teleport to="body">
            <div v-if="isInventorModalVisible" class="create-team-modal-container">
                <div class="modal-overlay" @click.self="hideInventorModal">
                    <div class="modal-content-dark">
                        <h2>添加发明人</h2>
                        <div class="modal-scroll-container">
                            <div class="form-group">
                                <label for="inventorName">发明人姓名:</label>
...
const fetchPatents = async (page = 1) => {
    try {
        const response = await patentAPI.get_my_patent(page)
        patents.value = response.data.results
...
const submitPatent = async () => {
    try {
        ...
        formData.append('name', newPatent.value.name);
        formData.append('number', newPatent.value.number);
        formData.append('date', newPatent.value.date);
        formData.append('patent_type', newPatent.value.patent_type);
        formData.append('description', newPatent.value.description);

        // 添加文件
        if (newPatent.value.file) {
            formData.append('file', newPatent.value.file);
        }

        // 添加发明人信息为JSON字符串
        formData.append('inventors', JSON.stringify(newPatent.value.inventors));

        const response = await patentAPI.create_patent(formData);
        console.log(response);
        if (response.data.code == 200) {
            ElMessage.success('专利创建成功');
        } else {
            ElMessage.error('专利创建失败');
        }
        closeModal();
        fetchPatents();
    } catch (error) {
        console.error('创建专利失败:', error);
        ElMessage.error('创建专利失败');
    }
}
// 移除发明人
const removeInventor = (index) => {
    newPatent.value.inventors.splice(index, 1)
}
...
const confirmAddInventor = () => {
    newPatent.value.inventors.push({
